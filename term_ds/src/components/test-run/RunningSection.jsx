...
import { getCarPriority, getSignForCell, setCarsToPlace, signDirectionsMap, specificCarPathsMap } from "./utils/roadUtils";
...
export default function RunningSection({ questionData, questionIndex, setScore, setCanGoNext, children }) {
  ...
  const carPaths = useMemo(() => {
    return specificCarPathsMap(questionData, carsData, routesData); // Створюємо карту всіх маршрутів для кожного питання, використовуючи функцію specificCarPathsMap,
  ...
  const indicators = useMemo(() => (
    getDirectionIndicators(carsToPlace, carPaths, grid, routesData[questionData.id]) // Викликається лише тоді, коли змінюються carsToPlace, carPaths або grid.
...
}, [carsToPlace, grid, signList, blockedDirections, conditionsData.signs, carPaths, carPriorities]);
                       ...
                       if (carRow === rowIndex && carCol === colIndex) {
                         ...
                         return (
                           <div
                             ...
                             //  className={`car-visual ${carId} ${key}`}
                             data-type={carId}
                             data-car-key={key}
                             style={{
                               ...
                               left: "50%",
                               position: "absolute",
                               opacity: isFinished ? 0.6 : 1, // Візуально показуємо завершені машинки
                               cursor: isFinished ? "not-allowed" : "pointer",
                             }}
                             onClick={(e) => {
                               e.stopPropagation(); // Запобігаємо спливанню події на клітинку

                               if (isFinished) {
                                 return toast.info(
                                   "🚗 Ця машинка вже завершила рух"
                                 );
                               }
